...
>> gcc expm_lapack_dgeev_2.c -lblas -llapack -lm  -L/usr/lib -o out

>> gcc expm_lapack_dgeev_2.c -lblas -llapack -lm  -o out
...
/* calculate the product B = VR*D , Column major order*/  
 for (i = 0; i < N; ++i) {
      ...
      while( j < N ) {
        ...
        if( WI[j] == (double)0.0 ) {
           B[i + N*j] = VR[i + N*j] * exp_lambda_R - VR[i + N*(j+1)] * exp_lambda_I ;  /* calculate the product B = VR*D */
            j++;
         } else {
           B[i + N*j] = VR[i + N*j] * exp_lambda_R - VR[i + N*(j+1)] * exp_lambda_I ;  /* calculate the product B = VR*D */
           B[i + N*(j+1)] = VR[i + N*j] * exp_lambda_I + VR[i + N*(j+1)] * exp_lambda_R ;  
...
/* Print B */
   print_complex_matrix( "Matrix B =  VR * expm(lambda)", N, WI, B, N );

/*Rearrage B array into BB complex array, Column major order*/
   double complex *BB =  malloc(N*N * sizeof(double complex));
   for( i = 0; i < N; i++ ) {
      j = 0;
      while( j < N ) {
         if( WI[j] == (double)0.0 ) {
            ...
            BB[i + j*N] = B[i + j*N] + I*B[i + (j + 1)*N] ;           
            BB[i + (j + 1)*N] = B[i + j*N] - I*B[i+(j + 1)*N] ;          
            j += 2;
         }
      }
   }

/* Print BB complex array, Column major order*/
   print_matrix( "Matrix BB =  VR * expm(lambda)", N, N, BB, N );
...
/* Find Inverse matrix of a complex array*/        
...
/* Calculate the product A = B*inv(VR) = VR*D*inv(VR)  */
   ...
   cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, m, n, k, &alpha, BB, LDA, invVR, LDB, &beta, C, LDC);
...
/*                        subprograms                        */
/*************************************************************/
/* Auxiliary routine: Inverse complex matrix*/
...
/* Auxiliary routine: printing a complex matrix A */
...
/* Auxiliary routine: printing a real matrix A */
...
/* Auxiliary routine: printing eigenvector */
...
/* Auxiliary routine: allocate 2D matrix  */
